...
_context_records_cache: dict[Path, tuple[int, int, dict[str, ContextRecord]]] = {}
_context_records_lock = threading.Lock()
...
def load_context_cache(path: Path | None = None) -> dict[str, ContextRecord]:
    ...
    try:
        ...
        stamp = (-1, 0)
    with _context_records_lock:
        cached = _context_records_cache.get(target)
    ...
    try:
        ...
        pass
    with _context_records_lock:
        _context_records_cache[target] = (stamp[0], stamp[1], records)
    return dict(records)
