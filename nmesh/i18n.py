...
MESSAGES = {
    "en": {
        ...
        "warn.ollama_max_loaded_models": "OLLAMA_MAX_LOADED_MODELS={value} is below the plan's {count} ollama service(s); the shared daemon will evict a loaded model on additional requests — silent reload churn inside request latency.",
    ...
    "warn.plan_stale": "保存済みのプランは古い nmesh で生成されました（起動フラグの仕様が変更されています）。最新の改善を取り込むには `nmesh plan` を再実行してください。",
        ...
        "warn.ollama_max_loaded_models": "OLLAMA_MAX_LOADED_MODELS={value} はプランの ollama サービス数 {count} を下回っています。共有デーモンが追加リクエストのたびにロード済みモデルを退避します（リクエスト内で静かなリロードが発生）。",
