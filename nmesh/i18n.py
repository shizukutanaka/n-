...
MESSAGES = {
    "en": {
        ...
        "warn.vllm_cpu_offload": "{service}: vLLM will run {model} with {gib} GiB of weights parked on the CPU (--cpu-offload-gb) as a virtual VRAM extension; the shown decode speed is a modeled estimate until measured.",
    ...
    "ja": {
        ...
        "warn.vllm_cpu_offload": "{service}: vLLM は {model} の重み {gib} GiB を CPU に置きます(--cpu-offload-gb、仮想VRAM拡張)。表示のデコード速度は実測まで推定値です。",
