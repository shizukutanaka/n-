...
MESSAGES = {
    "en": {
        ...
        "warn.gateway_env_key_drift": "{path}: existing gateway.env is kept, but its NMESH_API_KEY differs from the current environment; the gateway will keep requiring the stored value. Delete the file to regenerate it.",
    ...
    "ja": {
        ...
        "warn.gateway_env_key_drift": "{path}: 既存の gateway.env を保持しますが、ファイル内の NMESH_API_KEY が現在の環境と異なります — ゲートウェイは保存済みの値を要求し続けます。再生成するにはファイルを削除してください。",
