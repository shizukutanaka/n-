...
MESSAGES = {
    "en": {
        ...
        "warn.backend_log_unavailable": "Backend log unavailable for {service}: {error}; continuing without a log file",
    ...
    "warn.plan_stale": "保存済みのプランは古い nmesh で生成されました（起動フラグの仕様が変更されています）。最新の改善を取り込むには `nmesh plan` を再実行してください。",
        ...
        "warn.backend_log_unavailable": "{service} のバックエンドログを取得できません: {error} — ログファイルなしで継続します",
