...
MESSAGES = {
    "en": {
        ...
        "warn.resolution_env": "Environment variables that change module/library resolution are set and are inherited by every spawned process: {vars}. Python services (mlx server, gateway) and native engines may resolve different code or libraries than probing verified — unset them if the divergence is unintended.",
    ...
    "ja": {
        ...
        "warn.resolution_env": "モジュール/ライブラリ解決を変更する環境変数が設定されており、spawn される全プロセスに継承されます: {vars}。Python 系サービス（mlx サーバ・gateway）とネイティブエンジンが、プローブが検証したものとは別のコード/ライブラリを解決し得ます。意図と異なる場合は解除してください。",
