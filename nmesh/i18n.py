...
MESSAGES = {
    "en": {
        ...
        "warn.context_shift_swa": "{model}: context shift requested but this model's sliding-window cache cannot shift KV entries — llama.cpp disables it, so --context-shift is omitted.",
    ...
    "ja": {
        ...
        "warn.context_shift_swa": "{model}: コンテキストシフトが要求されましたが、このモデルのスライディングウィンドウキャッシュは KV エントリをシフトできないため llama.cpp では無効化されます — --context-shift は省略します。",
