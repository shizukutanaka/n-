...
MESSAGES = {
    "en": {
        ...
        "warn.cache_reuse_swa": "{model}: --cache-reuse omitted — llama.cpp cannot shift KV entries on sliding-window-attention models (its reduced SWA cache drops out-of-window tokens) and ignores the flag.",
    ...
    "ja": {
        ...
        "warn.cache_reuse_swa": "{model}: --cache-reuse を省略 — llama.cpp はスライディングウィンドウ注意のモデルでは KV エントリをシフトできず（縮小 SWA キャッシュはウィンドウ外トークンを破棄）フラグを無視します。",
