...
MESSAGES = {
    "en": {
        ...
        "warn.ollama_host_overridden": "OLLAMA_HOST={host} is set, but nmesh targets a fixed loopback daemon; the spawned ollama serve is pinned to 127.0.0.1:11434. Unset OLLAMA_HOST to silence this warning.",
    ...
    "warn.plan_stale": "保存済みのプランは古い nmesh で生成されました（起動フラグの仕様が変更されています）。最新の改善を取り込むには `nmesh plan` を再実行してください。",
        ...
        "warn.ollama_host_overridden": "OLLAMA_HOST={host} が設定されていますが、nmesh は固定のループバックデーモンを対象とするため、起動する ollama serve は 127.0.0.1:11434 にピン留めされます。この警告を消すには OLLAMA_HOST を解除してください。",
