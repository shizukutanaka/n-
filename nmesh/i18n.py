...
MESSAGES = {
    "en": {
        ...
        "warn.mlx_kv_unbounded": "{model}: this mlx_lm.server lacks --max-kv-size; each request's KV cache grows without bound, so memory may exceed the plan's budget on long inputs.",
    ...
    "ja": {
        ...
        "warn.mlx_kv_unbounded": "{model}: この mlx_lm.server は --max-kv-size に対応していないため、各リクエストの KV キャッシュが無制限に増大し、長い入力でプランの予算を超過する可能性があります。",
