...
MESSAGES = {
    "en": {
        ...
        "warn.ollama_env": "Environment variables are set and apply to the shared Ollama daemon and every ollama service: {vars}. They are respected as user configuration — unset them if the divergence is unintended.",
    ...
    "warn.plan_stale": "保存済みのプランは古い nmesh で生成されました（起動フラグの仕様が変更されています）。最新の改善を取り込むには `nmesh plan` を再実行してください。",
        ...
        "warn.ollama_env": "環境変数が設定されており、共有 Ollama デーモンとすべての ollama サービスに適用されます: {vars}。ユーザ設定として尊重されますが、意図と異なる場合は解除してください。",
