...
MESSAGES = {
    "en": {
        ...
        "warn.kv_quant_v_fa": "{model}: --cache-type-v {kv_quant} requires llama.cpp flash attention; on models where FA is unavailable the server refuses to start — use --kv-quant f16 if startup fails.",
    ...
    "ja": {
        ...
        "warn.kv_quant_v_fa": "{model}: --cache-type-v {kv_quant} は llama.cpp の flash attention が必須です。FA 非対応のモデルではサーバが起動を拒否します — 起動に失敗する場合は --kv-quant f16 を指定してください。",
