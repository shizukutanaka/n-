...
MESSAGES = {
    "en": {
        ...
        "warn.upstream_env_conflict": "{env} is set in your environment and the spawned {service} inherits it; nmesh expects an unauthenticated plain-HTTP loopback upstream, so requests to {service} will fail until {env} is unset.",
    ...
    "warn.plan_stale": "保存済みのプランは古い nmesh で生成されました（起動フラグの仕様が変更されています）。最新の改善を取り込むには `nmesh plan` を再実行してください。",
        ...
        "warn.upstream_env_conflict": "{env} が環境に設定されており、起動する {service} が継承します。nmesh は認証なしの平文 HTTP ループバック上流を前提とするため、{env} を解除するまで {service} へのリクエストは失敗します。",
