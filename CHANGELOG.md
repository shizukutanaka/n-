...
||||||| parent of b19ed81 (remove the LLM-router routing mode from DESIGN — it was never implemented)
- DESIGN.md のルーティング規則から「LLMルーター（`routing.mode: llm`）」の記述を削除しました。planner は常に `mode="rules"` を出力し、gateway は `mode` を読まないため、記載されていた任意モードは存在しませんでした。plan.json の `mode` フィールド自体はスキーマ互換のため残します。
