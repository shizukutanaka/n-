...
||||||| parent of 35a2423 (remove nmesh autotune: fixed 4-cell grid, restarts services and rewrites plan.json)
...
- `nmesh autotune` を削除しました。実行内容は「context 半分/等倍 × n_gpu_layers 半分/等倍」の固定 4 セル計測で結果は実測ベンチから事前に予測可能でありながら、稼働中サービスの再起動と plan.json の書き換えを伴い、計画が静かに恒久化されるリスク（#134 で修正した不具合系統と同型）を持っていました。証拠ベースの計画には `nmesh bench` の実測値で十分であり、選ばれる値はプランナーが既に行っています。
