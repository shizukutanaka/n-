...
### Fixed
- **`nmesh autotune` が detached ゲートウェイの watchdog と競合して失敗する問題を修正**: watchdog は15秒周期で heartbeat を回し、autotune がセル計測のため `runtime_down` → `runtime_up` で再起動した直後のサービスを「クラッシュ」と見て復活させ、次セルの起動が `port N is still in use` で失敗していました（実機で確認）。autotune はグリッド開始前に実行中のゲートウェイを `stop_gateway(foreign=True)` で停止し、全 exit パスで計測後に `--detach` で再起動するようにしました
