...
- **plan.json hot-swap trick**: a detached gateway's watchdog re-reads `$NMESH_HOME/plan.json` on mtime change (~15s heartbeat) and converges: kills engines whose model no longer matches, spawns new-plan services, and stops owned services the plan dropped. To verify a different model without `down/up`: rewrite plan.json `model_ref` AND the `-m` path in `launch.argv`, wait ~15-30s, confirm via `status` + listener port. Relative `NMESH_HOME` is absolutized against cwd — always export an absolute path to be unambiguous.
- `nmesh unload` is gateway-admin-mediated (`/admin/unload` POST): it marks the service idle on the GATEWAY supervisor so the watchdog skips respawning it — it does not fight the watchdog. `--sleep-idle-seconds` is unrelated: a llama-server argv flag (engine-side sleep), not the nmesh unload/idle state.
- Smoke home currently serves `Qwen_Qwen3-1.7B-Q5_K_M.gguf` on chat:18010 (0.6B Q8_0/bf16 also on disk for fallback).
- **spec-drift convergence**: the supervisor records the argv it spawned (`launched_argv`) and restarts a still-running service whose plan launch spec changed — so a plan.json swap that changes model/quant/flags kills the old engine and respawns the new spec (~15s heartbeat). Adopted engines are checked via the recorded state entry's `model_ref` instead.
- **`nmesh status` merges the live gateway `/status`**: gateway-memory-only states (`failed` reason, `idle` after unload) are merged into the CLI table — a `failed` service shows its reason below the table, an unloaded service shows `stopped` + `chat: idle`. If the gateway is down, the CLI view silently degrades to state.json only.
